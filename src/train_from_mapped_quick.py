"""
Quick version of train_from_mapped.py - uses a smaller sample for faster testing
Use this to test quickly, then use train_from_mapped.py for full training
"""
...
# QUICK VERSION: Limit number of rows for faster training
MAX_ROWS = 50000  # Use 50K rows instead of all 551K - much faster!
...
def main():
    print("=" * 60)
    print("QUICK TRAINING VERSION - Using sample of data")
    ...
    print(f"Loading dataset (max {MAX_ROWS:,} rows)...")
    # Load in chunks to save memory, or just read first N rows
    df = pd.read_csv(PROCESSED, nrows=MAX_ROWS)
    ...
    # QUICK VERSION: Fewer estimators and simpler models
    candidates = {
        ...
        "RF":     make_pipe(RandomForestClassifier(
                        n_estimators=100,  # Reduced from 400 to 100 for speed
    ...
    # QUICK VERSION: 3-fold CV instead of 5-fold
    skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=RANDOM_STATE)

    print("Model comparison (CV on train, then test on holdout):")
    print("(QUICK VERSION - using 3-fold CV and fewer estimators)")
    best_name, best_pipe, best_cv = None, None, -1.0
    for name, pipe in candidates.items():
        print(f"Training {name}...")
        cv_scores = cross_val_score(pipe, X_train, y_train, cv=skf, n_jobs=-1)
